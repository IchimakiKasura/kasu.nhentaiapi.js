...
class main {
    ...
    */
    ...
    getID(link) {
        //transformation
        /[${url}g/]/.test(link)?link=parseInt(link.replace(`${url}g/`,"")):typeof(link)!='number'?link=parseInt(link):null;
        return {
            json: async (data) => {
                const list = await shorter(link,this.IsDiscord,null,null,this.blockedWords)
    ...
    async pRandTag(string, data){
        return this.#pRand(string, data, "pRandTag", "tag")
    };
    async pRandParody(string, data){
        return this.#pRand(string, data, "pRandParody", "parody")
    };
    async pRandArtist(string, data){
        return this.#pRand(string, data, "pRandArtist", "artist")
    };
    async pRandGroup(string, data){
        return this.#pRand(string, data, "pRandGroup", "group")
    };
