...
function randrange(int) {
    if (typeof (int) != 'number') throw 'input value is not an integer!';return Math.floor(Math.random()*int)+1
}
...
async function pRCL(tag, string){
    return cheerio.load(await fetcher(`${url}${tag}/${string}/`))
}
// 
...
class main {
    ...
    getID(ID,discordToS) {
        ...
        return {
            list: async (data = {}) => {
                return data(await shorter(ID,discordToS))
            },
            json: async () => {
                let val = await shorter(ID,discordToS)
                try{
                    return {
                        base: {
                            id: val.id,
    ...
    //multiple arg use
    async pRandSpecificTags(string, data, discordToS) {
        ...
        if ($('div.index-container>h2').text() == "No results found") {
            ...
            if (data == null) {
                ...
                return data(await shorter(link, discordToS))
            }
        }
    }

    //one arg use | spelling sensitive
    async pRandTag(string, data, discordToS) {
    ...
    //one arg use | spelling sensitive
    async pRandParody(string, data, discordToS) {
        string = replacer(string,"-")
    ...
    //one arg use | spelling sensitive
    async pRandArtist(string, data, discordToS) {
        string = replacer(string,"-")
    ...
    //one arg use | spelling sensitive
    async pRandGroup(string, data, discordToS) {
        string = replacer(string,"-")
