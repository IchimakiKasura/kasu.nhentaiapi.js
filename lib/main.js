...
// less spaces less size :D
let jsonData=(idVal)=>{
return {id:idVal.id,url:idVal.url,
title: {origin:idVal.title_original,translated:idVal.title},
images: {cover:idVal.cover,pages:idVal.page_pics},
tag_table: {parodies:idVal.parodies,characters:idVal.characters,tag:idVal.tags,artist:idVal.artist,groups:idVal.group,languages:idVal.language,categories:idVal.category,},
number_pages: idVal.pages,
uploaded: idVal.uploaded
}
}
...
class main {
    ...
    async #pRandDataGiver($,string,tag,data,bool,tag2,spTag,spLink){
        ...
        try{
            return data(jsonData(await shorter(link, this.IsDiscord,this.ReRollonFail, string)))
        } catch(e) {
    ...
    getID(ID) {
        ...
        return {
            json: async (data) => {
                const list = jsonData(await shorter(ID,this.IsDiscord)) 
    ...
    async pRandom(data) {
        ...
        if (!data) {
            ...
            try{
                return data(jsonData(await shorter(dataId, this.IsDiscord,this.ReRollonFail)))
            } catch(e) {
