        ...
        return new Promise((response, error)=>{
            ...
            client.request(options)
                .on('response', headers=>{
                    ...
                    if(Debug) log(status)
                    if(status > 400) {
                        log('Too much internet pawah too fast boi\nstill no proxy to get away with this')
                        process.exit(0)
                    }
                    if(status > 300 && status < 400){
                        ...
                        client.request(options)
                            ...
                            .on('end',()=>{
                                ...
                                /*********/
                                /***/debugIteration("[PARSER.JS | FETCHER] redirect took to iterate: ",performance.now() - start)
                                /*********/
                            })
                            .end()
                    }
                ...
                .on('end',()=>{
                    if(status == 200){
                        response(str.replace('undefined',''))
                        /*********/
                        /***/debugIteration("[PARSER.JS | FETCHER] took to iterate: ",performance.now() - start)
                        /*********/
                    }
                })
                .end()
...
class Parser extends Request{
    ...
    static async Shorter(url, IsDiscord, RRoF, userString, blockedWords, IgnoreNone, client){
        ...
        /*********/
        /***/debugIteration("[PARSER.JS | SHORTER] took to iterate: ",performance.now() - start)
        /*********/
        return list
    }
